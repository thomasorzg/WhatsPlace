    ...
    public function saveBusinessSettings(Request $request)
    {
        ...
        if(\Auth::user()->type == 'super admin')
        {
            if($request->logo)
            {
                ...
                $path     = $request->file('logo')->storeAs('uploads/store_logo/', $logoName);
                \DB::insert(
        ...
        else if(\Auth::user()->type == 'Owner')
        {
            if($request->company_logo)
            {
                ...
                \DB::insert(
                    'insert into settings (`value`, `name`,`created_by`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`) ', [                                                                                                                                                 $logoName,                                                                                                                                                'company_logo',                                                                                                                                                 \Auth::user()->current_store,
                                                                                                                                             ]
                );
            }
