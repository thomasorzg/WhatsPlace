...
<head>
    ...
    <meta name="csrf-token" content="{{ csrf_token() }}">
        <title>{{(App\Models\Utility::getValByName('title_text')) ? App\Models\Utility::getValByName('title_text') : config('app.name', 'Whatsstore Saas')}}</title>
                                    ...
                                    tab_collaps_html +=`<form enctype="multipart/form-data" id="form_system_new_content_${li_id}">
                                                <div class="col-lg-12">
                                                    <div class="tab-modal-img">
                                                        ...
                                                        <img alt="data-1" src="{{ asset('landing/images/default-thumbnail.jpg') }}" id="system_img_new_content_${li_id}">
                ...
                <div class="section_div_{{$value->id}} tooltip1 ui-widget-content" id="{{$value->id}}">
                    @if($value->section_type == "section-plan")
                        <script>
                            $('#{{$value->id}}').load(`{{ url("/get_landing_page_section") }}/{{$value->section_blade_file_name}}`,function() {
                                ...
                                $(`#{{$value->id}}`).find('#delete').click(function() {delete_content("{{$value->id}}")});
                    ...
                    @else
                        ...
                        <script>
                            ...
                            $('#{{$value->id}}').find('#copy_btn').attr('data-id', '{{$value->id}}');
                            show_content('{{$value->id}}');
                            $(`#setting-modal-section-{{$value->id}}`).find('#save').click(function() {add_content("{{$value->id}}")});
                            $(`#{{$value->id}}`).find('#delete').click(function() {delete_content("{{$value->id}}")});
                        </script>
                    @endif
                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                @if (count($get_section) > 0)
                    @foreach ($get_section as $key => $value)
                        @if ($value->content == "" || $value->content == null)
                            <img src="{{asset('storage/Landing_page_component_image')}}/{{$value->section_demo_image}}" onclick="set_section_page('{{$value->id}}')" id="component_modal_{{$value->id}}">
                        @endif
                    @endforeach
                @endif
            </div>
